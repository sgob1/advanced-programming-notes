...
se hanno un diverso parametro di ritorno). In particolare,
...
maniera efficiente l'IDE mediante autocompletamento. Solitamente, nel nome del
metodo è insito che cosa faccia, tenendo anche conto delle \textbf{naming
conventions}.
...
una buona \textbf{qualità del codice}. La scelta corretta dei nomi, dunque, è
...
dell'oggetto. Ciò avvviene poiché la classe \texttt{String} è
\emph{immutabile}.}
    \label{fig:snippet59}
\end{figure}
...
\emph{stato} di un oggetto e il \emph{codice} della classe potrebbe cambiare,
...
\begin{description}
  ...
  \item[Costruttore] Si dice costruttore della classe \texttt{T} un metodo
      ...
      automaticamente introdotto da Java qualora non fosse stato esplicitamente
      dichiarato. Il costruttore di default è invece \textbf{assente} nel caso
      in cui vengano dichiarati altri costruttori con argomento diverso.
      L'invocazione del costruttore avviene mediante la parola chiave
      \texttt{new}, specifica del linguaggio Java, ed obbligatoria nella quasi
      totalità dei casi, eccezion fatta per oggetti della classe
      \texttt{String}.
  \item[Information hiding] L'\emph{information hiding} è il principio mediante
      ...
      drastiche variazioni di design abbiano luogo.
\end{description}
...
oggetto ne rappresentano lo \textbf{stato}. Uno stato è un possibile valore di
un'istanza appartenente all'universo dei possibili valori di un tipo. I field
sono referenziati, e sono dunque anche dotati di un identificatore. I field
hanno un tipo ed un nome del riferimento scelto (identificatore). Essi possono
essere manipolati tramite la dot notation, in egual maniera che i metodi. Ad
esempio,
...
\begin{itemize}
    ...
    \item \emph{default}: (identificatore assente) visibile.
\end{itemize}
...
codice, lettura del codice, o altri effetti pratici se non quello di rendere più agevole la scrittura del codice.}. Con \texttt{import}, dichiaro l'uso
...
necessari al momento della scrittura del codice. Il Fully Qualified Name è, a
volte, necessario nel caso di due \emph{classi omonime}, appartenenti a
...
mai cambiare al runtime. Sono cosiddetti ``0-based indexing''\footnote{Questo assunto, che per uno studentello di C parrebbe ovvio, non è affatto scontato: sia il linguaggio \textsc{Matlab} che \texttt{R} fanno uso di array ``1-based'', cioè che contano a partire da 1.}. Ciascun oggetto è accessibile
...
Abbiamo discusso dello scopde delle variabili, ora invece affronteremo lo scope
...
il modificatore \texttt{protected} rende visibile \texttt{solo nella classe che
